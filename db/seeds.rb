...
thnc.stock_prices.create(price: price_in_dollars(2.0), time: Time.current - 2.days)
bmo.stock_prices.create(price: price_in_dollars(5.0), time: Time.current - 2.days)
noodle.stock_prices.create(price: price_in_dollars(10.0), time: Time.current - 2.days)

thnc.stock_prices.create(price: price_in_dollars(1.0), time: Time.current - 1.day)
bmo.stock_prices.create(price: price_in_dollars(6.0), time: Time.current - 1.day)
noodle.stock_prices.create(price: price_in_dollars(15.0), time: Time.current - 1.day)

portfolio.trades.create(
  stock: thnc, trade_type: :buy, time: Time.current,
  quantity: 100, price: price_in_dollars(2.0)
)
portfolio.trades.create(
  stock: bmo, trade_type: :buy, time: Time.current,
  quantity: 100, price: price_in_dollars(5.0)
)
portfolio.trades.create(
